...
& javac --release 11 -classpath $AJAR -d obj src\com\rev\displayoff\DisplayOff.java
if ($LASTEXITCODE -ne 0) { throw "javac failed" }

Write-Host "[3/6] d8"
$classes = @(Get-ChildItem "obj\com\rev\displayoff\*.class" | ForEach-Object { $_.FullName })
...
& "$BT\apksigner.bat" sign --ks $KS --ks-pass pass:android --key-pass pass:android `
    --out out\DisplayOff.apk out\aligned.apk
if ($LASTEXITCODE -ne 0) { throw "apksigner failed" }

Write-Host "BUILD OK -> $PROJ\out\DisplayOff.apk"
Get-Item out\DisplayOff.apk | Select-Object Name,Length
